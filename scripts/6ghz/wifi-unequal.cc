              ...
              WifiPreamble preamble)
{
  ...
  snr = RatioToDb (snr);
  NS_LOG_INFO ("PHY-RX-OK time=" << Simulator::Now () << " node=" << ContextToNodeId (context)
                                 << " size=" << p->GetSize ()
                                 << " nMPDUs=" << p->GetSize () / pktSize << " snr=" << snr
...
public:
  ...
  int Run (const WifiHelper &wifi, const YansWifiPhyHelper &wifiPhy, const WifiMacHelper &wifiMac,
           ...
           double duration, bool pcap, bool infra, uint16_t channelWidth, uint16_t guardIntervalNs,
...
Experiment::Run (const WifiHelper &helper, const YansWifiPhyHelper &wifiPhy,
                 ...
                 uint16_t channelWidth, uint16_t guardIntervalNs, double distance, double apTxPower,
                 double staTxPower, uint16_t pktInterval)
{
  ...
  if (infra)
    {
      ...
      uint64_t beaconInterval = std::min<uint64_t> (
          (ceil ((duration * 1000000) / 1024) * 1024),
  ...
  Config::Set (
      ...
      BooleanValue (guardIntervalNs == 400 ? true : false));
  ...
  double angle = (static_cast<double> (10) / (nNodes - 1));
  ...
  for (uint32_t i = 1; i < nNodes; ++i)
    {
      uint32_t j = 0;
      PacketSocketAddress socketAddr;
      socketAddr.SetSingleDevice (devices.Get (j)->GetIfIndex ());
      socketAddr.SetPhysicalAddress (devices.Get (i)->GetAddress ());
      socketAddr.SetProtocol (1);

      Ptr<PacketSocketClient> client = CreateObject<PacketSocketClient> ();
      client->SetRemote (socketAddr);
      wifiNodes.Get (j)->AddApplication (client);
      client->SetAttribute ("PacketSize", UintegerValue (pktSize));
      client->SetAttribute ("MaxPackets", UintegerValue (0));
      //client->SetAttribute ("Interval", TimeValue (Time("0.00001")));
      client->SetAttribute ("Interval", TimeValue (MilliSeconds (1)));
      double start = startTime->GetValue ();
      NS_LOG_DEBUG ("STA client " << i << " starting at " << start);
      client->SetStartTime (Seconds (start));

      Ptr<PacketSocketServer> server = CreateObject<PacketSocketServer> ();
      server->SetLocal (socketAddr);
      wifiNodes.Get (i)->AddApplication (server);
    }
  ...
  if (standard == "11a")
  ...
  else if (standard == "11n")
    {
      if (frequency == 2.4)
        ...
        {
          NS_FATAL_ERROR ("Unsupported frequency band " << frequency << " MHz for standard "
                                                        << standard);
        }
    }
      ...
      for (uint32_t runIndex = 0; runIndex < trials; runIndex++)
        {
          ...
          experiment.Run (wifi, wifiPhy, wifiMac, wifiChannel, runIndex, n, duration, pcap, infra,
                          channelWidth, guardIntervalNs, distance, apTxPower, staTxPower,
                          pktInterval);
          uint32_t k = 0;
  ...
  for (iter = snrMap.begin (); iter != snrMap.end (); iter++)
    {
      if (tracing)
        {
          snrTraceFile << "SNR is: " << iter->first << "; Total number is: " << iter->second
                       << std::endl;
        }
